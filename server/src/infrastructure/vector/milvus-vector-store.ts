...
export class MilvusVectorStore implements VectorStore {
  ...
  public async ensureCollection(): Promise<void> {
    ...
    if (!exists.value) {
      await this.client.createCollection({
        ...
        fields: [
          ...
          // FIXED: source_id is the stable canonical identifier used for source filtering
    ...
    try {
      ...
      // Index may already exist from a previous boot.
    }
  ...
  public async search(
    ...
    const searchParams: any = {
      ...
      limit,
      output_fields: [
        'id', 'content', 'source_id', 'source_file', 'source_type',
        'page_number', 'chunk_index', 'start_char', 'end_char',
        'metadata', 'embedding_model', 'session_id',
      ],
      params: { nprobe: 16 },
    };
    ...
    return rows.map((row: any) => {
      ...
      return {
        ...
        citation: {
          ...
          sourceType,
          pageNumber: Number(entity.page_number ?? row?.page_number ?? -1) === -1
            ? null
            : Number(entity.page_number ?? row?.page_number),
          chunkIndex: Number(entity.chunk_index ?? row?.chunk_index ?? 0),
          startChar: Number(entity.start_char ?? row?.start_char ?? -1) === -1
            ? null
            : Number(entity.start_char ?? row?.start_char),
          endChar: Number(entity.end_char ?? row?.end_char ?? -1) === -1
            ? null
            : Number(entity.end_char ?? row?.end_char),
        },
        metadata:
          typeof (entity.metadata ?? row?.metadata) === 'object' && (entity.metadata ?? row?.metadata)
  ...
  public async getById(id: string): Promise<Record<string, unknown> | null> {
    ...
    const rows: any = await this.client.query({
      collection_name: this.collectionName,
      filter: `id == "${id}"`,
      output_fields: ['id', 'content', 'metadata', 'source_id', 'source_file', 'source_type', 'page_number', 'chunk_index'],
    });
