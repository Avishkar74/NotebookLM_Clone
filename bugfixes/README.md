...
IngestionService.ingest*()
  ...
  │
  ├── ResilientEmbedder
  │   ├── Primary: OpenAI text-embedding-3-small (384 dim)
  │   └── Fallback: LocalHashEmbedder (384 dim, char-frequency)
  │
...
Fix: Ensure `CHATANYWHERE_API_KEY` and `BASE_URL` are set correctly. The `ResilientEmbedder` will attempt OpenAI first.
...
|----------|----------|-------------|
| `CHATANYWHERE_API_KEY` | ✅ | OpenAI-compatible API key |
